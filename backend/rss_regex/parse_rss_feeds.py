...
async def process_source(source: DataSources) -> list[dict]:
    ...
    On failure, returns a single RssItem with the error status so we track every attempt.
    """
...
async def run(run_id: str | None = None, disconnect_after: bool = True):
    """Main entry point for RSS parsing.
    ...
    try:
        ...
        results = await asyncio.gather(
            *(process_with_limit(src) for src in sources), return_exceptions=True
        )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(f"Error processing {sources[i].name}: {result}")
            else:
